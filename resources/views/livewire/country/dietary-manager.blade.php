...
<div class="space-y-6 pb-12" dir="{{ $locale === 'ar' ? 'rtl' : 'ltr' }}">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm">
        <div class="flex items-center gap-4">
            <div class="w-14 h-14 rounded-2xl bg-amber-500/10 text-amber-600 flex items-center justify-center font-black shrink-0 border border-amber-500/20 shadow-sm">
            ...
            <div>
                <h1 class="text-xl sm:text-2xl font-black text-[#06205C] dark:text-white tracking-tight">
                    {{ $t('سجل حساسية الطعام والاحتياجات الغذائية للوفد', 'Registre des Allergies & Régimes Alimentaires', 'Delegation Food Allergies & Dietary Register') }}
                </h1>
                <p class="text-xs font-bold text-slate-500 dark:text-slate-400 mt-1">
                    {{ $country?->getLocalized('name') ?? $t('الوفد الأفريقي', 'Délégation Africaine', 'African Delegation') }} — {{ $t('إدارة وتوثيق حساسية الطعام والاحتياجات الطبية والغذائية الخاصة لجميع أعضاء الوفد لتأمين الوجبات بالمطاعم.', 'Gestion et suivi des régimes alimentaires et allergies pour la restauration.', 'Manage and record dietary restrictions and food allergies for all delegation members.') }}
                </p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-2">
            <button onclick="window.print()" class="px-4 py-2.5 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs shadow-md transition flex items-center gap-2">
                ...
                <span>{{ $t('طباعة كشف الإطعام الصحي', 'Imprimer le rapport', 'Print Catering Report') }}</span>
            </button>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
        ...
        <div class="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xs text-center space-y-1">
            <span class="text-slate-400 text-[10px] font-black uppercase tracking-wider block">
                {{ $t('إجمالي أعضاء الوفد', 'Total Membres', 'Total Members') }}
            </span>
        ...
        <div class="bg-amber-50 dark:bg-amber-950/40 p-4 rounded-2xl border border-amber-200/80 dark:border-amber-900/60 shadow-xs text-center space-y-1">
            ...
            <span class="text-[10px] font-bold text-amber-700 dark:text-amber-400">
                {{ $totalMembers > 0 ? round(($membersWithAllergiesCount / $totalMembers) * 100) : 0 }}% {{ $t('من الوفد', 'du total', 'of delegation') }}
            </span>
        </div>
    ...
    <div class="bg-white dark:bg-slate-800 p-4 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-3">
        ...
        <div class="text-xs font-bold text-slate-400 shrink-0">
            {{ $t('المعروض: ', 'Affichés: ', 'Showing: ') }} <span class="text-[#06205C] dark:text-white font-black">{{ $members->count() }}</span>
        </div>
    </div>

    {{-- MEMBERS DIETARY TABLE --}}
                        ...
                        <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-700/50 transition">
                            ...
                            <td class="p-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-2xl bg-[#06205C]/10 text-[#06205C] dark:bg-sky-500/20 dark:text-sky-300 font-black flex items-center justify-center shrink-0 border border-[#06205C]/20 uppercase">
                                    ...
                                    <div>
                                        ...
                                        <span class="text-[11px] font-mono text-slate-400 block">{{ $m->passport_number ?: 'DZ-OFFICIAL' }}</span>
                                    </div>
                                </div>
                            </td>
                            ...
                            {{-- Role --}}
                            <td class="p-4">
                                <span class="px-2.5 py-1 rounded-full text-[10px] font-black uppercase border bg-slate-100 dark:bg-slate-900 text-slate-800 dark:text-slate-200 border-slate-200 dark:border-slate-700">
                                    {{ $m->member_type }}
                                </span>
                            </td>
