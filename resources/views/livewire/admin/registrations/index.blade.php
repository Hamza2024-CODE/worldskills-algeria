...
<div class="space-y-8 pb-12" dir="{{ $locale === 'ar' ? 'rtl' : 'ltr' }}">
        ...
        <div class="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div class="space-y-2">
                ...
                <h1 class="text-2xl sm:text-4xl font-black text-white tracking-tight">
                    سجل طلبات الترشح والتسجيلات الوطنية والدولية
                </h1>
                
                <p class="text-xs sm:text-sm text-slate-300 font-bold max-w-2xl">
                    متابعة واعتماد ملفات المترشحين والحكام، طباعة القوائم المعتمدة حسب الولايات والدول، واستخراج التقارير التنفيذية.
                </p>
            </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-white dark:bg-slate-800 rounded-3xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm space-y-2">
            <span class="text-xs font-black text-slate-500 dark:text-slate-400 block uppercase">إجمالي طلبات التسجيل</span>
            <span class="text-3xl font-black text-slate-900 dark:text-white tracking-tight block">{{ number_format($totalCount) }}</span>
            <span class="text-[11px] font-bold text-slate-400 block">كافة الملفات المودعة بالنظام</span>
        </div>

        <div class="bg-white dark:bg-slate-800 rounded-3xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm space-y-2">
            <span class="text-xs font-black text-emerald-600 dark:text-emerald-400 block uppercase">مقبولة رسمياً (Approved)</span>
            ...
            <span class="text-[11px] font-bold text-emerald-700 dark:text-emerald-500 block">{{ $totalCount > 0 ? round(($approvedCount/$totalCount)*100, 1) : 0 }}% من الإجمالي</span>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-3xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm space-y-2">
            ...
            <span class="text-[11px] font-bold text-amber-700 dark:text-amber-500 block">تستلزم التدقيق والمراجعة</span>
        </div>
    ...
    {{-- ═════════════════════════════════════════════════════════════════════
         4. COMPREHENSIVE 6-WAY FILTERS HUB (الولاية، الدولة، التخصص، الحالة، الدور)
    ═════════════════════════════════════════════════════════════════════ --}}
    <div class="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm space-y-4">
        ...
        <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-700 pb-3">
            <h3 class="text-xs font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest flex items-center gap-2">
                ...
                <span>تصفية وفلترة سجلات التسجيل بالكامل</span>
            </h3>
    ...
    {{-- ═════════════════════════════════════════════════════════════════════
         5. REGISTRATIONS LIST TABLE GRID
    ═════════════════════════════════════════════════════════════════════ --}}
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-start text-xs">
                <thead class="bg-slate-50 dark:bg-slate-900/60 border-b border-slate-200 dark:border-slate-700 text-slate-500 font-bold uppercase">
                    <tr>
                        <th class="px-5 py-4 text-start">المترشح / صاحب التسجيل</th>
                        <th class="px-5 py-4 text-start">التخصص والمهارة</th>
                        <th class="px-5 py-4 text-start">الولاية / الدولة</th>
                        <th class="px-5 py-4 text-start">القياسات</th>
                        <th class="px-5 py-4 text-center">حالة الطلب</th>
                        <th class="px-5 py-4 text-end">إجراءات الاعتماد</th>
                    </tr>
                </thead>
                        ...
                        <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-700/40 transition">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-3">
                                    <img 
                                        src="{{ $photoUrl }}" 
                                        alt="{{ $p?->first_name_ar }}" 
                                        class="w-11 h-11 rounded-xl object-cover border border-slate-200 dark:border-slate-700 shrink-0 shadow-xs"
                                    />
                            ...
                            <td class="px-5 py-4">
                                <div class="text-[11px] text-slate-600 dark:text-slate-300 space-y-0.5">
                                    <span>بدلة: <strong class="text-slate-900 dark:text-white">{{ $reg->suit_size ?? '—' }}</strong></span> | 
                                    <span>حذاء: <strong class="text-slate-900 dark:text-white">{{ $reg->shoe_size ?? '—' }}</strong></span>
                                </div>
                            </td>

                            {{-- Status Badge --}}
                            <td class="px-5 py-4 text-center">
                                <span class="px-3 py-1 rounded-xl text-[11px] font-black {{ $statusBadge }}">
                                    {{ $statusVal }}
                                </span>
                            </td>
                            ...
                            <td class="px-5 py-4 text-end">
                                <div class="flex items-center justify-end gap-1.5">
                                    ...
                                    <button 
                                        wire:click="openStatusModal({{ $reg->id }})" 
                                        title="تحديث حالة الاعتماد" 
                                        class="p-2 rounded-xl bg-blue-50 hover:bg-blue-100 dark:bg-blue-950 text-[#0066FF] dark:text-blue-300 transition">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-slate-400 font-bold">
                                لم يتم العثور على أي نتائج مطابقة لفلاتر التسجيلات المحددة.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    {{-- ═════════════════════════════════════════════════════════════════════
         6. REGISTRATION DETAILS DRAWER
    ═════════════════════════════════════════════════════════════════════ --}}
    @if($drawerOpen && $selectedRegistration)
        @php
            $sp = $selectedRegistration->participant;
            $sStatus = $selectedRegistration->status instanceof \App\Enums\ParticipantStatus ? $selectedRegistration->status->value : $selectedRegistration->status;
        @endphp
        <div class="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm flex items-center justify-end p-4">
            <div class="bg-white dark:bg-slate-900 rounded-3xl max-w-lg w-full h-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-6 overflow-y-auto">
                ...
                <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
                    <div class="flex items-center gap-3">
                        <img src="{{ $selectedRegistration->photo_url }}" alt="{{ $sp?->first_name_ar }}" class="w-14 h-14 rounded-2xl object-cover border border-slate-200 dark:border-slate-700">
                        <div>
                ...
                <div class="space-y-4 text-xs font-bold">
                    <div class="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 flex items-center justify-between">
                        <span class="text-slate-500">حالة الاعتماد بالحساب:</span>
                        <span class="px-3 py-1 rounded-xl text-xs font-black {{ $sStatus === 'APPROVED' ? 'bg-emerald-100 text-emerald-800' : ($sStatus === 'REJECTED' ? 'bg-rose-100 text-rose-800' : 'bg-amber-100 text-amber-800') }}">
                            {{ $sStatus }}
                        </span>
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-3 text-center">
                        <div class="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/40">
                            ...
                            <span class="font-black text-slate-900 dark:text-white text-sm mt-0.5 block">{{ $selectedRegistration->suit_size ?? '—' }}</span>
                        </div>
                        <div class="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/40">
                            <span class="text-slate-400 block text-[10px]">قياس الحذاء</span>
                            <span class="font-black text-slate-900 dark:text-white text-sm mt-0.5 block">{{ $selectedRegistration->shoe_size ?? '—' }}</span>
                        </div>
                        <div class="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/40">
                            <span class="text-slate-400 block text-[10px]">القامة (سم)</span>
                            <span class="font-black text-slate-900 dark:text-white text-sm mt-0.5 block">{{ $selectedRegistration->height_cm ? $selectedRegistration->height_cm . ' سم' : '—' }}</span>
                        </div>
                    </div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm">
            <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 w-full max-w-md shadow-2xl border border-slate-200 dark:border-slate-800 space-y-4">
                <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                    <h3 class="text-base font-black text-slate-900 dark:text-white">تحديث حالة طلب التسجيل</h3>
                    <button wire:click="$set('statusModalOpen', false)" class="text-slate-400 hover:text-slate-600 font-black">✕</button>
                </div>

                <div class="space-y-3 text-xs font-bold">
                    <label class="block text-slate-700 dark:text-slate-300">اختر الحالة الجديدة *</label>
                    <select wire:model="newStatus" class="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white font-bold">
                        @foreach($statuses as $st)
                            <option value="{{ $st->value }}">{{ $st->value }}</option>
                        @endforeach
                    </select>
                ...
                <div class="flex items-center justify-end gap-2 pt-3 border-t border-slate-100 dark:border-slate-800">
                    <button wire:click="$set('statusModalOpen', false)" class="px-4 py-2 text-xs font-bold text-slate-600 dark:text-slate-400 rounded-xl">إلغاء</button>
                    <button wire:click="saveStatus" class="px-6 py-2.5 text-xs font-black text-white bg-blue-600 hover:bg-blue-700 rounded-xl shadow-md">تحديث الحالة</button>
                </div>
            </div>
        </div>
    @endif
    ...
    @if($rejectModalOpen)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm">
            <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 w-full max-w-md shadow-2xl border border-slate-200 dark:border-slate-800 space-y-4">
                <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                    <h3 class="text-base font-black text-rose-600">سبب رفض طلب التسجيل</h3>
                    <button wire:click="$set('rejectModalOpen', false)" class="text-slate-400 hover:text-slate-600 font-black">✕</button>
                </div>

                <div class="space-y-3 text-xs font-bold">
                    <label class="block text-slate-700 dark:text-slate-300">اكتب سبب الرفض الإلزامي *</label>
                    <textarea wire:model="rejectionReason" rows="3" placeholder="مثال: الصورة شخصية غير واضحة، أو رقم NIN غير مطابق..." class="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white font-bold"></textarea>
                </div>
                ...
                <div class="flex items-center justify-end gap-2 pt-3 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <button wire:click="saveRejection" class="px-6 py-2.5 text-xs font-black text-white bg-rose-600 hover:bg-rose-700 rounded-xl shadow-md">تأكيد الرفض</button>
                </div>
            </div>
        </div>
    @endif
    ...
    @if($deleteConfirmOpen)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm">
            <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 w-full max-w-sm shadow-2xl border border-slate-200 dark:border-slate-800 space-y-4 text-center">
                <h3 class="text-base font-black text-rose-600">تأكيد حذف طلب التسجيل</h3>
                <p class="text-xs text-slate-500 font-bold">هل أنت تأكد من رغبتك في حذف هذا التسجيل نهائياً من قاعدة البيانات؟ لا يمكن التراجع عن هذا الإجراء.</p>
                ...
                <div class="flex items-center justify-center gap-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <button wire:click="deleteRegistration" class="px-6 py-2.5 text-xs font-black text-white bg-rose-600 hover:bg-rose-700 rounded-xl shadow-md">تأكيد الحذف</button>
                </div>
            </div>
        </div>
    @endif
