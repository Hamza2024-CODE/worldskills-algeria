        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
            <div>
                <h1 class="text-xl font-black text-[#06205C]">
                    <x-ws.icon name="printer" class="w-5 h-5 inline-block me-1.5" /> {{ app()->getLocale() === 'fr' ? 'Impression Groupée des Badges' : (app()->getLocale() === 'en' ? 'Bulk PVC Badges Printing' : 'الطباعة الجماعية لبطاقات الاعتماد الرسمية') }}
                </h1>
            ...
            <div class="flex items-center gap-3">
                ...
                <button type="button" onclick="window.print()" class="px-6 py-2.5 rounded-xl bg-[#0066FF] hover:bg-[#0052CC] text-white font-black text-xs shadow-lg transition flex items-center gap-2">
                    ...
                    <span>{{ app()->getLocale() === 'fr' ? 'Lancer l\'Impression Groupée (Print All)' : (app()->getLocale() === 'en' ? 'Print All Badges' : 'طـبـاعة جميع البطاقات الآن') }}</span>
                </button>
            </div>
        </div>
