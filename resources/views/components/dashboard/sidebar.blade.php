...
<aside
    ...
    class="hidden lg:flex bg-white/75 dark:bg-[#0B1120]/80 backdrop-blur-2xl border-e border-slate-200/60 dark:border-slate-800/60 flex-col shrink-0 h-[calc(100vh-64px)] sticky top-16 transition-all duration-300 ease-in-out z-30 select-none shadow-[4px_0_24px_rgba(0,0,0,0.02)]"
>

    {{-- Collapse/Expand Floating Button --}}
    <button
        @click="toggle()"
        class="absolute top-2.5 -end-3.5 z-50 w-7 h-7 rounded-full bg-white/95 dark:bg-slate-800/95 backdrop-blur-md border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-200 flex items-center justify-center shadow-md hover:scale-110 hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-slate-700 transition-all cursor-pointer"
    ...
    {{-- Quick Search Input Capsule --}}
    <div x-show="!collapsed" class="p-3 pt-2.5 pe-4 pb-2 border-b border-slate-200/50 dark:border-slate-800/60">
        <div class="relative">
            <input type="text"
                   x-model="search"
                   placeholder="{{ $locale === 'fr' ? 'Rechercher...' : ($locale === 'en' ? 'Quick Search...' : 'تصفية وبحث سريع...') }}"
                   class="w-full pl-8 pr-3 py-2 rounded-full text-xs bg-slate-100/75 dark:bg-slate-800/75 backdrop-blur-md border border-slate-200/60 dark:border-slate-700/60 text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/40 transition shadow-2xs">
    ...
    <nav class="flex-1 overflow-y-auto overflow-x-hidden p-3 space-y-3.5 scrollbar-none">
        ...
        @foreach($categorizedNav as $catIndex => $group)
            ...
            <div x-data="{ open: true }"
                ...
                {{-- Category Section Label Capsule --}}
                <div x-show="!collapsed"
                     @click="open = !open"
                     class="flex items-center justify-between px-3 py-1.5 rounded-full cursor-pointer hover:bg-slate-100/70 dark:hover:bg-slate-800/60 transition group/cat">
                    <div class="flex items-center gap-2">
                        <span class="w-2 h-2 rounded-full bg-[#0066FF] shrink-0"></span>
                        <span class="text-[11px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-wider">
                            {{ $catName }}
                        </span>
                    </div>
                ...
                <div x-show="open || search !== ''" class="space-y-1 pt-0.5">
                    @foreach($groupItems as $item)
                        ...
                        <a href="{{ $href }}"
                           ...
                           class="relative flex items-center gap-3 py-2.5 rounded-full text-xs font-bold transition-all duration-200 group/item {{ $isActive ? 'bg-[#0052CC] text-white font-black shadow-md shadow-blue-500/30' : 'text-slate-700 dark:text-slate-300 hover:bg-white/80 dark:hover:bg-slate-800/80 hover:text-[#0052CC] dark:hover:text-[#38C6FF] hover:shadow-2xs' }}"
                           @if($isActive) style="background: linear-gradient(135deg, #0052CC 0%, #0077FF 100%) !important; color: #ffffff !important;" @endif
                        >
                            {{-- Icon Wrapper with Strict Size --}}
                            <div class="w-5 h-5 flex items-center justify-center shrink-0">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                     class="w-5 h-5 shrink-0 transition-transform duration-200 group-hover/item:scale-110 {{ $isActive ? 'text-white' : 'text-slate-500 dark:text-slate-400 group-hover/item:text-[#0052CC]' }}" aria-hidden="true">
                                    {!! $svgPath !!}
                                </svg>
                            </div>
                            ...
                            @if($isActive)
                                <span class="absolute end-2 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-white animate-pulse"></span>
                            @endif

                            {{-- Floating Tooltip when Collapsed --}}
                            <div x-show="collapsed"
                                 x-cloak
                                 class="absolute {{ app()->getLocale() === 'ar' ? 'right-full mr-2' : 'left-full ml-2' }} px-3 py-2 rounded-2xl text-[11px] font-bold text-white bg-slate-900/95 dark:bg-slate-800/95 backdrop-blur-xl border border-slate-700 shadow-xl pointer-events-none opacity-0 group-hover/item:opacity-100 transition-opacity duration-150 whitespace-nowrap z-50">
                                {{ $item['label'] }}
                            </div>
                        </a>
                    @endforeach
            ...
            <div x-show="!collapsed" class="border-t border-slate-100/70 dark:border-slate-800/60 my-1"></div>
        @endforeach

    </nav>

    {{-- Bottom Profile User Summary Capsule --}}
    <div class="p-3 border-t border-slate-200/60 dark:border-slate-800/60 bg-white/40 dark:bg-slate-900/40 backdrop-blur-md">
        <a href="{{ route('profile') }}"
           :class="collapsed ? 'justify-center px-0' : 'px-2.5 py-1.5'"
           class="w-full flex items-center gap-2.5 rounded-full text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-800 border border-transparent hover:border-slate-200 dark:hover:border-slate-700 transition-all shadow-2xs group"
        >
            <div class="w-8 h-8 rounded-full bg-[#06205C] overflow-hidden shrink-0 border border-slate-200 dark:border-slate-700 shadow-2xs flex items-center justify-center text-white font-black text-xs">
